...
import time
# from bounding_box import get_roi as rois
import pandas as pd
import datetime
...
def draw_boundary(img, roi):
    for _, x1, y1, x2, y2 in roi:
...
def detect(weights=ROOT / 'yolov5n.pt',
        ...
        blur_obj=False,color_box = False,roi=ROOT / 'source/out.txt'):
    ...
    """
    TIPS: 
    PRESS 0 = DRAW ROI FOR CASHIER
    PRESS 1 = DRAW ROI FOR CUSTOMER
    PRESS q WHEN DONE DRAWING
    """

    """
    path = path of video
    im = tensor in numpy array format
    im0s = image
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        # print(f"the dt is {dt[0]} {dt[1]} {dt[2]}")

        """ EVERY FRAME ONE BY ONE
        i is each iterator count
        det is in 1x5 numpy array formate --> [x1, y1, x2, y2, conf, class]
        """
        for i, det in enumerate(pred):  # per image
            ...
            # print(f"width {width} height {height} other {ROIs}")

            # for _, xn, yn, wn, hn in ROIs:
            #     x1 = int(xn * width)
            #     y1 = int(yn * height)
            #     x2 = int((xn + wn) * width)
            #     y2 = int((yn + hn) * height)

            #     normalized_roi.append([_, x1, y1, x2, y2])

            draw_boundary(im0, ROIs)        
        ...
        # print(f"Path is {path}, im is {im}, im0s is{im0s}, vid_cap is {vid_cap} and s is {s}")
        print(s)
    ...
    time_db = iters.get_time_database()
